...
function renderResumeStatus(status: string) {
  const colors: { [index: string]: 'success' | 'warning' | 'info' } = {
    'In Queue': 'warning',
    Analyzing: 'info',
    Scored: 'success',
  };
  ...
  return (
    <Chip label={status} color={colors[status]} size="small" variant="outlined" />
  );
}

export const resumeDataColumns: GridColDef[] = [
  { field: 'id', headerName: 'Resume Id', flex: 1, minWidth: 50 },
  {
    field: 'status',
    headerName: 'Status',
    headerAlign: 'center',
    ...
    flex: 1,
    minWidth: 50,
    renderCell: (params) => renderResumeStatus(params.value as any),
  },
  {
    field: 'impact',
    headerName: 'Impact',
...
export const resumeDataRows: GridRowsProp = [
  {
    id: 1,
    status: 'In Queue',
    impact: '-',
